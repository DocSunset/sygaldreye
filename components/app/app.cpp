...
static void acquire_multicast_lock(struct android_app* app) {
    ...
    jmethodID mkLock = env->GetMethodID(
        wifiClass,
        "createMulticastLock",
...
void android_main(struct android_app* app) {
    ...
    while (!app->destroyRequested) {
        if (state.xrInstance == XR_NULL_HANDLE) state.xrInstance = xr_create_instance(app);
        ...
        if (ALooper_pollOnce(33, nullptr, &events, reinterpret_cast<void**>(&source)) >= 0 &&
            source)
            source->process(app, source);
        offline_tick();
    }
    if (app->destroyRequested) {
        LOG("android_main: exiting (pre-XR)");
        return;
    }
    ...
    while (!app->destroyRequested && !state.xrSession.should_quit()) {
        ...
        if (state.xrSession.session_running()) {
            state.frame_loop_.run_frame(state.xrSession.get(), [&](XrTime t) -> FrameLayers {
                ...
                // The editor is graph content (editor + wire_mesh nodes fed
                ...
                FrameLayers result;
                result.layers.at(0) = reinterpret_cast<const XrCompositionLayerBaseHeader*>(
                    &state.renderer.proj_layer());
                result.count = 1;
                return result;
            });
        }
