...
struct EditorNode {
    ...
    static consteval std::string_view source_header() {
        return "components/editor_node/editor_node.hpp";
    }
    ...
    struct endpoints {
        ...
        out<DrawFn> render;
        out<Span> wires;  // N×10 {from.xyz, to.xyz, rgba} → wire_mesh
    } endpoints;

    void operator()(double time_s);

    void set_context(const Graph* g, const ComponentRegistry* r, EventQueue<std::string>* edits) {
        graph_ = g;
        registry_ = r;
        edits_ = edits;
    }
