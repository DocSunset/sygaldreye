...
template <typename N>
concept HasSourceHeader = requires {
    { N::source_header() } -> std::convertible_to<std::string_view>;
};

template <typename N>
concept HasSourceCpp = requires {
    { N::source_cpp() } -> std::convertible_to<std::string_view>;
};

template <typename N>
concept HasName = requires {
    { N::name() } -> std::convertible_to<std::string_view>;
};

// v7: optional lifting metadata. A node opts out of the stateful default by
// declaring `static constexpr int lift_kind()` (an EyeballsLiftKind) and/or
// `static constexpr std::string_view lift_key()` (keyed-identity field).
template <typename N>
concept HasLiftKind = requires {
    { N::lift_kind() } -> std::convertible_to<int>;
};

template <typename N>
concept HasLiftKey = requires {
...
constexpr std::string_view v6_kind() {
    if constexpr (std::same_as<T, GpuTexture>)
        return "texture";
    else if constexpr (std::same_as<T, MeshPtr>)
        return "mesh";
    else if constexpr (std::same_as<T, MeshArray>)
        return "mesh_array";
...
void for_each_named_field(S& s, Fn&& fn) {
    boost::pfr::for_each_field(
        s, [&]<std::size_t I>(auto& field, std::integral_constant<std::size_t, I>) {
...
const EyeballsNodeDescriptor* make_descriptor() {
    ...
    if constexpr (HasProcess<Node>) {
        process_fn = [](void* p, double t) { (*static_cast<Node*>(p))(t); };
    }
    ...
    static void (*set_texture_in_fn)(
        void*, const char*, unsigned int, int, int, unsigned int, unsigned int) = nullptr;
    ...
    if constexpr (HasEndpoints<Node>) {
        static std::string v6_schema = [] {
            std::string ins = "[", outs = "[";
            Node tmp{};
            detail::for_each_named_field(tmp.endpoints, [&](auto& f, std::string_view nm) {
                using F = std::remove_cvref_t<decltype(f)>;
                auto add = [&](std::string& s, std::string_view kind, const std::string& extra) {
                    if (s.size() > 1) s += ',';
                    s += "{\"name\":\"";
                    s += nm;
                    s += "\",\"kind\":\"";
                ...
                if constexpr (V6EventIn<F>)
                    add(ins, "bang", "");
                else if constexpr (V6EventOut<F>)
                    add(outs, "bang", "");
                else if constexpr (V6Whole<F>)
                    // lifting opt-out: the schema marks this port whole-array.
                    add(ins, detail::v6_kind<typename F::value_type>(), ",\"whole\":1");
        ...
        push_outputs_fn = [](void* p, EyeballsOutputCtx* ctx) {
            ...
            detail::for_each_named_field(node->endpoints, [&](auto& f, std::string_view nm) {
                using F = std::remove_cvref_t<decltype(f)>;
                if constexpr (V6EventOut<F>) {
                    ctx->emit_scalar(ctx->store, ctx->node_id, nm.data(), f.triggered ? 1.0 : 0.0);
                } else if constexpr (V6Output<F>) {
                    using T = typename F::value_type;
                    if constexpr (std::same_as<T, MeshPtr>) {
                        ...
                        ctx->emit_mat4(ctx->store, ctx->node_id, nm.data(), f.value.data());
                    } else if constexpr (std::same_as<T, Eigen::Quaternionf>) {
                        ctx->emit_quat(
                            ctx->store,
                            ctx->node_id,
                        ...
                        ctx->emit_vec2(
                            ctx->store, ctx->node_id, nm.data(), f.value.x(), f.value.y());
                    } else if constexpr (std::same_as<T, Span>) {
                        if (ctx->emit_span)
                            ctx->emit_span(
                                ctx->store,
                                ...
                                f.value.rows,
                                f.value.cols,
                                int(f.value.row_axis),
                                int(f.value.col_axis));
                    } else if constexpr (std::same_as<T, std::string>) {
                        if (ctx->emit_text)
                            ctx->emit_text(ctx->store, ctx->node_id, nm.data(), f.value.c_str());
                    } else if constexpr (std::is_arithmetic_v<T>) {
                        ctx->emit_scalar(
                            ctx->store, ctx->node_id, nm.data(), static_cast<double>(f.value));
                    }
                }
            });
        };
    ...
    // v7 lifting metadata: stateful default, no key, unless the node says so.
    static int lift_kind_val = EYEBALLS_LIFT_STATEFUL;
    if constexpr (HasLiftKind<Node>) lift_kind_val = Node::lift_kind();
    ...
    static EyeballsNodeDescriptor d{
        ...
        .deserialize = [](void* p, const char* json) { from_json(*static_cast<Node*>(p), json); },
