...
// One aurora curtain: a rippling translucent sheet, additively blended. An
// aurora is a subgraph of these (assets/graphs/aurora.json). Emits a static UV
// grid Mesh + a Surface whose vertex shader ripples it via uTime; color,
// placement, and motion are per-node param uniforms. GL lives in render_region.
struct AuroraCurtainNode {
    ...
    static consteval std::string_view source_header() {
        return "components/aurora_curtain/aurora_curtain.hpp";
    }
