...
struct VrEditor {
    ...
    int palette_pages() const {
        return std::max(
            1, (static_cast<int>(palette_types_.size()) + kPaletteRows - 1) / kPaletteRows);
    }

    struct GraphEdit {
        std::string new_graph_json;
    };
    ...
    // Wire endpoints as data (decomposition slice 1): appends one N×10 row
    // {from.xyz, to.xyz, rgba} per resolved edge, plus the ghost wire while
    // dragging. Rendered by the wire_mesh node, not by draw().
    void collect_wires(std::vector<float>& rows) const;
