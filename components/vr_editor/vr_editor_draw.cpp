...
struct PrimBatch {
    GLuint vao = 0, vbo = 0;
    std::map<std::array<float, 4>, std::vector<float>> tris;

    std::vector<float>& run(const Eigen::Vector4f& c) { return tris[{c.x(), c.y(), c.z(), c.w()}]; }
...
void VrEditor::collect_wires(std::vector<float>& rows) const {
    rows.clear();
    auto push = [&rows](
                    const Eigen::Vector3f& a, const Eigen::Vector3f& b, const Eigen::Vector4f& c) {
        rows.insert(
            rows.end(), {a.x(), a.y(), a.z(), b.x(), b.y(), b.z(), c.x(), c.y(), c.z(), c.w()});
    };
    ...
    for (const auto& edge : current_edges_) {
            ...
            if (card_ids_[ci] == edge.from_node) {
                for (const auto& h : output_handles_[ci]) {
                    if (h.port_name == edge.from_port) {
                        from_pos = h.world_pos;
                        found_from = true;
                        edge_kind = h.port_kind;
                        break;
                    }
                }
            }
        ...
        if (found_from && found_to) push(from_pos, to_pos, port_kind_color(edge_kind));
    }

    // Ghost wire during drag
    if (drag_state_ == DragState::Dragging)
        push(drag_from_pos_, controller_tip_, port_kind_color(drag_from_kind_));
}
...
void VrEditor::draw(const Eigen::Matrix4f& vp, const TextMesh& text) const {
    ...
    char hdr[32];
    std::snprintf(hdr, sizeof(hdr), "more... %d/%d", palette_page_ + 1, palette_pages());
    for (int r = 0; r <= kPaletteRows; ++r) {
        int idx = palette_page_ * kPaletteRows + r - 1;
        if (r > 0 && idx >= static_cast<int>(palette_types_.size())) break;
        float v = (static_cast<float>(r) + 0.5f) / static_cast<float>(kPaletteRows + 1);
        ...
        text.draw(r == 0 ? hdr : palette_types_[static_cast<size_t>(idx)].c_str(), vp * m);
    }
    ...
    for (int i = 0; i < static_cast<int>(node_cards_.size()); ++i) {
        ...
        for (const auto& h : input_handles_[si])
            draw_quad(h.world_pos, 0.006f, 0.006f, port_kind_color(h.port_kind), vp, shader_);
        ...
        for (const auto& sl : sliders_[si]) {
            ...
            bool hov = std::abs(delta.x()) < sl.width * 0.5f && std::abs(delta.y()) < 0.03f &&
                       std::abs(delta.z()) < 0.06f;
            draw_quad(
                {thumb_x, sl.world_pos.y(), sl.world_pos.z() + 0.004f},
                hov ? 0.006f : 0.004f,
                sl.height * (hov ? 0.85f : 0.6f),
                hov ? Eigen::Vector4f{1.0f, 0.85f, 0.3f, 1.0f}
                    : Eigen::Vector4f{0.7f, 0.8f, 1.0f, 1.0f},
                vp,
                shader_);
        }
    }

    // Wires are graph content now: collect_wires → editor.wires span →
    // wire_mesh node (vr_editor_decomposition.md slice 1).
    ...
    if (show_left_)
        draw_quad(
            ...
            trig_l_ ? Eigen::Vector4f{1.0f, 0.8f, 0.3f, 0.9f}
                    : Eigen::Vector4f{0.5f, 0.9f, 0.9f, 0.6f},
            vp,
            shader_);
    ...
    if (!hover_label_.empty()) {
        ...
        draw_quad(
            {hover_pos_.x() + 0.06f, hover_pos_.y() + 0.02f, hover_pos_.z() + 0.010f},
            0.06f,
            0.014f,
            {0.02f, 0.02f, 0.05f, 0.92f},
            vp,
            shader_);
