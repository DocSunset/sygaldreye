...
- Each executor owns a **monotonic clock** and publishes it as an output;
  ...
  kernels receive dt or per-sample increments (aut.kernel_contract).
...
before release; state moves by route (exe.migration); queued events survive swap
...
- **Quarantine**: fresh plugins realize in subprocess regions; promotion is
  trust policy (msh.graphs_vs_plugins). Fault handling is a capability package; a sealed
  movement carries none of it.

## Requirements

**tcf.mapping_guarantees** Each row of the table has a stress test:
queue N events cross-thread (no loss/dup/reorder — lng.event_discipline.bangs_exact_stateless); latch
mid-block write invisibility (exe.canonical_mappings.latch_at_boundary); ring bounded staleness; net
reconnect discipline (pkg.net_package.reconnect_discipline).
**tcf.swap_safety** 10,000 random swaps under load: no torn values, no
lost events, in-flight tick always completes (extends exe.migration.phase_continuous).
**tcf.clock_honesty** No wall-clock syscalls from any node
(grep + runtime intercept); executor clocks are monotonic; offset
translation slews (max slew rate asserted).
**tcf.fault_matrix** One test per fault class by region kind:
...
state with testimony written (extends abi.fault_declaration, sz.spawn_and_park).
**tcf.movement_austerity** A frozen movement build contains no fault
