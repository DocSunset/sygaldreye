...
int main() {
  ...
  // symbol_id (constexpr twin) agrees with the minted resident.
  static_assert(symbol_id("x").digest != 0);
  assert(symbol_id("x") == sx.id);
  ...
  bool threw = false;
  try { insert_or_get(env, {.data = (void*)"evil!", .type = SYMBOL.id, .id = sx.id,
                            .size = 5, .env = nullptr}); }
  ...
  bind(&child, SYMBOL.id, sx.id, f32.id);
  assert(find(&child, SYMBOL.id, sx.id)->id == f32.id);
  assert(find(&child, SYMBOL.id, sx.id)->data == get(env, f32.id)->data);   // deref'd
  const syg_handle_t* raw = peek(&child, SYMBOL.id, sx.id);
  assert(raw && raw->type == ref_type_id() && raw->data == nullptr && raw->id == f32.id);
  bind(&child, SYMBOL.id, sx.id, s.id);
  assert(find(&child, SYMBOL.id, sx.id)->id == s.id);
