...
/* v7: how the executor lifts this node over an excess-rank (array) input.
   ...
   STATELESS → loop the pure process over rows (CellMap), no per-row state.
   RESOURCE_HOLDER → lifting is a hard error (dac, editor, device streams). */
enum EyeballsLiftKind {
    EYEBALLS_LIFT_STATEFUL = 0,
    EYEBALLS_LIFT_STATELESS = 1,
    EYEBALLS_LIFT_RESOURCE_HOLDER = 2
};
...
typedef struct EyeballsOutputCtx {
    ...
    void (*emit_vec4)(
        void* store, const char* nid, const char* port, float x, float y, float z, float w);
    ...
    void (*emit_mesh)(void* store, const char* nid, const char* port, const void* mesh);
    void (*emit_audio)(
        void* store,
        const char* nid,
        const char* port,
    ...
    void (*emit_text)(void* store, const char* nid, const char* port, const char* utf8);
    /* v6: rank-<=2 float spans (positions, transforms, lists). row_axis/
       col_axis carry Axis identity (see sygaldry_endpoints Axis) so the
       axes survive the ABI boundary instead of decaying to anonymous. */
    void (*emit_span)(
        void* store,
        const char* nid,
        const char* port,
...
typedef struct {
    ...
    void (*set_texture_in)(
        ...
        unsigned int filter);
    void (*set_audio_in)(
        void* node, const char* port, const float* samples, int frames, int channels, int rate);
    ...
    /* v7: lifting metadata. lift_kind is an EyeballsLiftKind (default
       STATEFUL). lift_key names the input field carrying keyed identity for
       Clones migration when the lifted array reorders (NULL ⇒ match by
       index). */
    int lift_kind;
    const char* lift_key;
