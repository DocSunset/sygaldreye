...
| 7 | `syg compile` | the CMP session: every compile is a derivation-mode run of the REALIZED engine plan (shared core with `syg peer` — the ONE compiler; ADR-034). Full-recipe memo at the driver; the structural stage is a store derivation inside the choose hook (`structural_memo` = the abi structural counter stayed flat). `passes_run` is the executor's tick trace. Projection edits through the inverse map (conflicts on vanished targets), forks, lineage. Freeze ops retired until the freezer fuses realize output (FRZ, rung 8) |
| 8 | `syg render-frozen <so> <secs>` | dlopen a frozen artifact (extern "C" frozen_create/pump/destroy), stream raw f32le to stdout; `{"ns_per_block"}` stat on stderr (render-graph emits the same stat) |
| 8 | `syg render-swap <so> <secs>` | hot-reload (FRZ-1.1): graph on stdin renders interpreted for the first half, the .so loads LIVE at a block boundary and takes over the stream — one process, no restart |
| 8 | `syg peer` (new ops) | `app-cid`; `cat` {cid} → raw bytes; `unfreeze` {artifact} → walks provenance (artifact → execution → recipe → app cid) — ADR-014: unfreezing is reading the store |
...
| 9 | `syg mesh` | scripted N-peer session over REAL loopback sockets + the ADR-035 crypto suite (Ed25519 identity + crypto_kx handshake + XChaCha20-Poly1305 secretstream framing; varint wire-kind + dag-cbor body). Ops: `peer-key`, `pair {a,b}` (mutual acceptance), `revoke {peer,of}` (drop key + sever), `put {peer,bytes\|node,store?}`, `bind {peer,ref,cid}`, `advertise {peer,run?,serve?,subscribe?}`, `share-store {peer,name,keys[]}` (a store shared with a key SUBSET), `fetch {from,to,cid}` (codec-aware ingest + verify), `subscribe {from,to,ref}`, `place {from,to,type}`, `place-fallthrough {from,candidates[],type}` (visible refusal trail; unplaceable), `place-derivation {from,candidates[],graph,blocks?}` (worker by advertised capability; result by hash), `has {peer,cid}`, `read {peer,cid}`, `hello {from,to}`, `audit-log {peer}` (instantiated types), `capture {peer,bytes,route?}` (signed testimony) + `verify-capture {testimony}`, `set-contract {peer,contract,migrations[]}` + `admit-plugin {peer,contract}` (ABI-4 reachability), `set-policy {peer,trust[]}` + `ship-graph {from,to,graph}` + `ship-plugin {from,to,form?,artifact?/bytes,sign?,source?,toolchain?,as?}` + `plugin-provenance {peer,type}` (MSH-5 trust gate; native = real dlopen), `send-ops {from,to,ref,ops[]}` (OPS toward an arbiter) + `net-pair {from,to,provider,consumer,...}` (PKG-6 discipline over the real transport), `probe {to,payload?}` (raw unauthenticated TCP scan → `{bytes_received,refused}`). Session-level: `record` (FMT-4 wire transcript) and `discovery: static|mdns` (the MSH-7 seam — same map either way). A refusal is a typed `{ok:false, refused/error, reason}`; MSH-1..8, FMT-4, ABI-4 |
| 11 | `syg edit` | the graph editor held to its laws (ch. 9): a live plan whose arbiter takes gesture edit ops. Ops: `open {graph,warm?}`, `gesture {ops[],settle?}` (add_node/remove_node/add_edge/remove_edge/set_param — the same ops a gesture NODE emits), `undo {n}` (STRUCTURAL-snapshot undo — param drift folds into its structural step, EDR-3), `save`/`doc` (serialize captures DEFAULTS never live cell values, EDR-2), `value {route}`, `open-editor {editor,target,arbiter}` + `bang {route}` + `swap {id,type}` + `target-doc` (EDR-1: the palette subgraph edits a target's arbiter; swap+migrate live), `agent-drive {editor,target,arbiter,op_route,bang,script[]}` (EDR-7: each gesture arrives as an op event from a source node), `realized {graph}` (EDR-4: the execution view's mappings, the latch labeled compiler-inserted+replaceable), `regions {graph}` + `probe {graph,edges[],blocks?}` (EDR-8: values-surface tap, region inference untouched). EDR-1..8 |
| 11 | `syg walk` | the store browser on the astui model (ch. 9). Setup: `store {types?, graphs{ref:graph}, fanout{ref:count}}` (commits type nodes + graphs with a lock + binds refs). Ops: `ground` (frontier = named refs), `walk {path[]}` (follow steps from ground; deref links; a node's type name hops through the lock to its type node), `frontier {path,page?,page_size?}` (demand-driven, streamed pagination — a 100k-link node never materializes all), `value {path}`, `mark {paths[],links?,as?}` (the marked subgraph + hand links = a committed dataset), `open-mark {ref\|cid}`. EDR-5 |
| 11 | `syg walk` (transclusion) | `cid-of {ref}`, `rebind {ref,graph}` (the ref MOVES), `transclude {ref\|cid, route[]}` — a LIVE address (ref) re-resolves each call (subscription), a FIXED address (cid) is pinned (quotation). EDR-6.1 |
| 11 | `syg document` | the document layer (ch. 9). `roundtrip {source}` — decompose C++ into top-level segment nodes linked in sequence (a byte-covering structural split, brace/string/comment aware) and regenerate byte-identically. EDR-6.2, a property over fixtures/cpp_corpus/ |
| 12 | `syg conform` | the self-hosting closure (ch. 17). `succeed {name,of?,class,criteria?}` — declare a succession (origin/fix/additive/breaking) committing a supersedes-linked node; the class gate REJECTS an additive/fix that regresses a green predecessor criterion, naming it (ADR-032). `version {cid\|name}` — the derived M.m.p (breaking hops / additive since last breaking / fix since last additive), computed from the chain, stored nowhere. `resolve {name,at}` — ref-sugar name@M.m.p → the chain-walk hash. CNF-6 |
