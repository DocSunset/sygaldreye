...
| 9 | `syg mesh` | scripted N-peer session over REAL loopback sockets + the ADR-035 crypto suite (Ed25519 identity + crypto_kx handshake + XChaCha20-Poly1305 secretstream framing; varint wire-kind + dag-cbor body). Ops: `peer-key`, `pair {a,b}` (mutual acceptance), `revoke {peer,of}` (drop key + sever), `put {peer,bytes\|node,store?}`, `bind {peer,ref,cid}`, `advertise {peer,run?,serve?,subscribe?}`, `share-store {peer,name,keys[]}` (a store shared with a key SUBSET), `fetch {from,to,cid}`, `subscribe {from,to,ref}`, `place {from,to,type}`, `hello {from,to}`, `audit-log {peer}` (instantiated types), `probe {to,payload?}` (raw unauthenticated TCP scan → `{bytes_received,refused}`). A refusal is a typed `{ok:false, refused, reason}`; MSH-1..8, FMT-4, ABI-4 |
