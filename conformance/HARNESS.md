...
| 9 | `syg mesh` | scripted N-peer session over REAL loopback sockets + the ADR-035 crypto suite (Ed25519 identity + crypto_kx handshake + XChaCha20-Poly1305 secretstream framing; varint wire-kind + dag-cbor body). Ops: `peer-key`, `pair {a,b}` (mutual acceptance), `revoke {peer,of}` (drop key + sever), `put {peer,bytes\|node,store?}`, `bind {peer,ref,cid}`, `advertise {peer,run?,serve?,subscribe?}`, `share-store {peer,name,keys[]}` (a store shared with a key SUBSET), `fetch {from,to,cid}` (codec-aware ingest + verify), `subscribe {from,to,ref}`, `place {from,to,type}`, `place-fallthrough {from,candidates[],type}` (visible refusal trail; unplaceable), `place-derivation {from,candidates[],graph,blocks?}` (worker by advertised capability; result by hash), `has {peer,cid}`, `read {peer,cid}`, `hello {from,to}`, `audit-log {peer}` (instantiated types), `capture {peer,bytes,route?}` (signed testimony) + `verify-capture {testimony}`, `set-contract {peer,contract,migrations[]}` + `admit-plugin {peer,contract}` (ABI-4 reachability), `probe {to,payload?}` (raw unauthenticated TCP scan → `{bytes_received,refused}`). A refusal is a typed `{ok:false, refused/error, reason}`; MSH-1..8, FMT-4, ABI-4 |
